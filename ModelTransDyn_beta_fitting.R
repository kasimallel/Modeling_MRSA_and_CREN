...
ARB_model_1 <- function(times, state, parms){
  ...
  INF_U_f_p1 <- max((1050- Nt1_spec)*0.7*mu0_p1,0)
  INF_U_m_p1 <- max((1050- Nt1_spec)*0.7*(1-mu0_p1),0)
  INF_CR_f_p1 <- max((1050- Nt1_spec)*0.1194*mu1_p1,0)
  INF_CR_m_p1<- max((1050- Nt1_spec)*0.1194*(1-mu1_p1),0)
  INF_CS_f_p1<- max((1050- Nt1_spec)*0.18156*mu2_p1,0)
  INF_CS_m_p1 <- max((1050- Nt1_spec)*0.18156*(1-mu2_p1),0)
  INF_IMR_f_p1<- max((1050- Nt1_spec)*(caIha_p1)*mu3_p1*(1/8),0)
  INF_IMR_m_p1<- max((1050- Nt1_spec)*(caIha_p1)*(1-mu3_p1)*(1/8),0)
  INF_ISR_f_p1<- max((1050- Nt1_spec)*(caIha_p1)*mu4_p1*(1/8),0)
  INF_ISR_m_p1<- max((1050- Nt1_spec)*(caIha_p1)*(1-mu4_p1)*(1/8),0)
  INF_IMS_f_p1<- max((1050- Nt1_spec)*(caIha_p1)*(mu5_p1)*(1/8),0)
  INF_IMS_m_p1<- max((1050- Nt1_spec)*(caIha_p1)*(1-mu5_p1)*(1/8),0)
  INF_ISS_f_p1<- max((1050- Nt1_spec)*(caIha_p1)*(mu6_p1)*(1/8),0)
  INF_ISS_m_p1<- max((1050- Nt1_spec)*(caIha_p1)*(1-mu6_p1)*(1/8),0)
...
ggplot(mrsa_df_long_f, aes(x = time, y = value, colour = variable)) + 
  ...
  theme(legend.position = c(0.9, 1), # Adjust these values to move the legend inside the plot
        legend.justification = c(0, 1)) # Anchors the legend position; (0,1) is top-left
...
ggplot(data = mrsa_df, aes(x = time, y = N_t)) + 
  ...
  labs(title = "Total hospital population size over time",
       x = "Time (days)",
       y = "Total population size (Nt)") +
  theme(plot.title = element_text(hjust = 0.5))+ # Center the plot title
  scale_y_continuous(breaks = seq(0, 1000, by = 50), limits = c(NA, 1000))
...
ggplot(data = cre_df, aes(x = time, y = N_t)) + 
  ...
  labs(title = "Total hospital population size over time",
       ...
       y = "Total population size (Nt)") +
  theme(plot.title = element_text(hjust = 0.5))+ # Center the plot title
  scale_y_continuous(breaks = seq(0, 1050, by = 50), limits = c(850, 1200))
...
#Observed data
library(readxl)
ruta_archivo <- "/Users/lsh1807578/CISS Dropbox/kasim allel henriquez/B_Projects/AMR_BSI_BurdenChile/0_Article_AMR Transmission dynamics Chile & LMICs/0_BSIModelling/0_analysis/transmissionParam/Book1 copy.xlsx"
prevalence_data <- read_excel(ruta_archivo, sheet = 2)
observed_data1_t <- prevalence_data[,'Oxacilin']/100
observed_data1 <-0.399
observed_data1 <-0.1198
observed_data2 <- 0.1445 
observed_data2a_t <- prevalence_data[,'Meropenem']/100
observed_data2b_t <- prevalence_data[,'Ertapenem']/100
observed_data2a <- 0.257 
  observed_data2b <- 0.463 
times <- seq(from=0, to=365, by = 1)  # Simulate over a year

# --------------- # 
#      MRSA
# --------------- #
#cost function for MRSA
cost_function1 <- function(tau_p1,observed_data1,mrsa_model_output){
  mrsa_model_output <- ode(y=state1, times = times, func=ARB_model_1, parms=parameters1)
  # population
  N_te1 <- mrsa_model_output[,'U_f1']+mrsa_model_output[,'U_m1']+mrsa_model_output[,'CR_m1']+mrsa_model_output[,'CS_m1']+mrsa_model_output[,'IMR_m1']+mrsa_model_output[,'ISR_m1']+mrsa_model_output[,'IMS_m1']+mrsa_model_output[,'ISS_m1']+
    mrsa_model_output[,'CR_f1']+mrsa_model_output[,'CS_f1']+mrsa_model_output[,'IMR_f1']+mrsa_model_output[,'ISR_f1']+mrsa_model_output[,'IMS_f1']+mrsa_model_output[,'ISS_f1']
  N_te12 <- mrsa_model_output[,'CR_m1']+mrsa_model_output[,'CS_m1']+mrsa_model_output[,'IMR_m1']+mrsa_model_output[,'ISR_m1']+mrsa_model_output[,'IMS_m1']+mrsa_model_output[,'ISS_m1']+
    mrsa_model_output[,'CR_f1']+mrsa_model_output[,'CS_f1']+mrsa_model_output[,'IMR_f1']+mrsa_model_output[,'ISR_f1']+mrsa_model_output[,'IMS_f1']+mrsa_model_output[,'ISS_f1']
  
  #resistant, colonised at leat or infected
  I_rs <- mrsa_model_output[,'CR_m1']+mrsa_model_output[,'IMR_m1']+mrsa_model_output[,'ISR_m1']+mrsa_model_output[,'CR_f1']+mrsa_model_output[,'IMR_f1']+mrsa_model_output[,'ISR_f1']
  Infected_rs = I_rs
  I <- mrsa_model_output[,'CR_m1']+mrsa_model_output[,'IMR_m1']+mrsa_model_output[,'ISR_m1']+mrsa_model_output[,'CR_f1']+mrsa_model_output[,'IMR_f1']+mrsa_model_output[,'ISR_f1']
  Infected = I
  
  #prevalence
  fitted_values <- Infected_rs/N_te1
  
  #cost
  cost<-sum((observed_data1-fitted_values)^2)
  
  return(cost)
  
}
#initial guess for tau_1
initial_tau1<-0.01
# Using optim to estimate beta
fit_1 <- optim(par = initial_tau1, fn = cost_function1, observed_data = observed_data1, mrsa_model_output = mrsa_model_output, method = "Brent", lower = 0, upper = 1, control = list(maxit = 10000))
# Best fit beta
tau1_best_fit_mrsa <- fit_1$par
tau1_fit<- fit_1$value
fit_1b <- optim(par = initial_tau1, fn = cost_function1, observed_data = observed_data1, mrsa_model_output = mrsa_model_output, method = "L-BFGS-B", lower = 0, upper = 1, control = list(maxit = 10000))
# Best fit beta
tau1_best_fit_mrsab <- fit_1b$par
tau1_fitb<- fit_1b$value

# --------------- # 
#      CRE
# --------------- #
initial_tau2<-0.01
cost_function2 <- function(tau_p2,observed_data2,cre_model_output){
  cre_model_output <- ode(y=state2, times = times, func=ARB_model_2, parms=parameters2)
  # population
  N_te1 <- cre_model_output[,'U_f2']+cre_model_output[,'U_m2']+cre_model_output[,'CR_m2']+cre_model_output[,'CS_m2']+cre_model_output[,'IMR_m2']+cre_model_output[,'ISR_m2']+cre_model_output[,'IMS_m2']+cre_model_output[,'ISS_m2']+
    cre_model_output[,'CR_f2']+cre_model_output[,'CS_f2']+cre_model_output[,'IMR_f2']+cre_model_output[,'ISR_f2']+cre_model_output[,'IMS_f2']+cre_model_output[,'ISS_f2']
  N_te12 <- cre_model_output[,'CR_m2']+cre_model_output[,'CS_m2']+cre_model_output[,'IMR_m2']+cre_model_output[,'ISR_m2']+cre_model_output[,'IMS_m2']+cre_model_output[,'ISS_m2']+
    cre_model_output[,'CR_f2']+cre_model_output[,'CS_f2']+cre_model_output[,'IMR_f2']+cre_model_output[,'ISR_f2']+cre_model_output[,'IMS_f2']+cre_model_output[,'ISS_f2']
  
  #resistant, colonised at leat or infected
  I_rs2 <- cre_model_output[,'CR_m2']+cre_model_output[,'IMR_m2']+cre_model_output[,'ISR_m2']+cre_model_output[,'CR_f2']+cre_model_output[,'IMR_f2']+cre_model_output[,'ISR_f2']
  Infected_rs2 = I_rs2
  I2 <- cre_model_output[,'CR_m2']+cre_model_output[,'IMR_m2']+cre_model_output[,'ISR_m2']+cre_model_output[,'CR_f2']+cre_model_output[,'IMR_f2']+cre_model_output[,'ISR_f2']
  Infected = I2
  
  #prevalence
  fitted_values2 <- Infected_rs2/N_te1
  
  #cost
  cost2<-sum((observed_data2-fitted_values2)^2)
  
  return(cost2)
  
}

fit_2 <- optim(par = initial_tau2, fn = cost_function2, observed_data = observed_data2, cre_model_output = cre_model_output, method = "L-BFGS-B", lower = 0, upper = 1, control = list(maxit = 10000))
# Best fit beta
tau1_best_fit_cre <- fit_2$par
#0.3986551
